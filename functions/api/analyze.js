// Cloudflare Pages Function — proxy + prompt builder for Inception Labs API.
// File location maps to URL: /api/analyze
//
// Receives the user's questionnaire answers and percentile scores, builds a
// rich prompt, sends it to Mercury 2 via the Inception API, and returns the
// generated analysis as JSON.
//
// Set INCEPTION_API_KEY as a Secret in the Cloudflare Pages dashboard.
// Get a key (with 10M free tokens) at https://platform.inceptionlabs.ai/
...
/**
 * Build the user message containing all answers + scores in a clean format.
 */
function buildUserMessage(payload) {
  ...
  answers.forEach((q, i) => {
    const lines = []
    lines.push(`Q${i + 1} (${q.mode === 'multi' ? 'select all' : 'top one'}): ${q.title}`)
...
function validatePayload(p) {
  ...
  if (p.answers.length !== 20) return `Expected 20 answers, got ${p.answers.length}`
...
export async function onRequestPost(context) {
  ...
  if (!env.INCEPTION_API_KEY) {
    return jsonError(
      500,
      'configuration_error',
      'INCEPTION_API_KEY is not set. Add it as a Secret in the Cloudflare Pages dashboard.'
    )
  }
  ...
  try {
    ...
    return jsonError(502, 'upstream_unreachable', `Could not reach Inception API: ${err.message}`)
  }
  ...
  try {
    ...
    return jsonError(502, 'upstream_invalid_response', 'Inception API returned non-JSON.')
  }

  const analysis = data?.choices?.[0]?.message?.content?.trim() || ''
  if (!analysis) {
    return jsonError(502, 'empty_response', 'Inception API returned an empty analysis.')
  }
